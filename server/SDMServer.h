...
public:
    SDMServer(const SDMConfig& config)
        : config_(config), running_(false), server_socket_(-1),
          ...
          driver_manager_(nullptr), document_manager_(nullptr),
          incident_manager_(nullptr), analytics_engine_(nullptr),
          vision_manager_(nullptr), request_handler_(nullptr),
    ...
    bool initialize() {
        ...
        vision_manager_ = new VisionManager();
        if (!vision_manager_->initialize()) {
            cout << "    ! Vision system initialization failed (cameras unavailable)" << endl;
            delete vision_manager_;
            vision_manager_ = nullptr;
        } else {
            cout << "    ✓ Vision manager initialized" << endl;
        }
        
        trip_manager_ = new TripManager(*db_manager_, *cache_manager_, 
                                       *index_manager_, vision_manager_);
        ...
        driver_manager_ = new DriverManager(*db_manager_, *cache_manager_, 
                                           *index_manager_);
        document_manager_ = new DocumentManager(*db_manager_, *cache_manager_);
        incident_manager_ = new IncidentManager(*db_manager_, *cache_manager_, 
                                               document_manager_);
        
        cout << "    ✓ Feature modules initialized" << endl;
        
        cout << "  [7/9] Initializing analytics engine..." << endl;
        analytics_engine_ = new AnalyticsEngine(
            *db_manager_, *cache_manager_,
            trip_manager_, expense_manager_, vehicle_manager_, vision_manager_
        );
        cout << "    ✓ Analytics engine initialized" << endl;
        ...
        request_handler_ = new RequestHandler(
            ...
            *driver_manager_, *document_manager_, *incident_manager_,
            *analytics_engine_, vision_manager_
        );
